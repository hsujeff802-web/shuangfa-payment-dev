...
function applySystemName(){const name=getSystemName();const h=$('#systemNameHeader');if(h)h.textContent=name;document.title=`${name} V8.3 DEV Build 025.7・簽名語音與儲存修正版`;const loginTitle=document.querySelector('#loginSystemName');if(loginTitle)loginTitle.textContent=name;const apple=document.querySelector('meta[name="apple-mobile-web-app-title"]');if(apple)apple.setAttribute('content',name.slice(0,12))}
...
function imageToCompressed(data,max=1400,quality=.86){return new Promise(resolve=>{if(!data||!String(data).startsWith('data:image'))return resolve(data||'');const im=new Image();im.onload=()=>{try{const scale=Math.min(1,max/Math.max(im.width,im.height)),cv=document.createElement('canvas');cv.width=Math.max(1,Math.round(im.width*scale));cv.height=Math.max(1,Math.round(im.height*scale));cv.getContext('2d').drawImage(im,0,0,cv.width,cv.height);resolve(cv.toDataURL('image/jpeg',quality))}catch{resolve(data)}};im.onerror=()=>resolve(data);im.src=data})}
async function optimizeStoredPhotos(){let changed=0;for(const p of db.payments||[]){const old=[...(p.invoicePhotos||[])];const newer=[];for(const x of old){const y=await imageToCompressed(x,1400,.84);if(y&&y.length<x.length)changed++;newer.push(y)}p.invoicePhotos=newer;if(p.checkPhoto){const y=await imageToCompressed(p.checkPhoto,1600,.88);if(y.length<p.checkPhoto.length)changed++;p.checkPhoto=y}if(p.signatureData){const y=await imageToCompressed(p.signatureData,1200,.90);if(y.length<p.signatureData.length)changed++;p.signatureData=y}}return changed}
async function saveWithStorageRecovery(currentId){try{return save()}catch(err){if(err?.code!=='STORAGE_QUOTA'&&err?.message!=='STORAGE_QUOTA')throw err;toast('儲存空間不足，正在壓縮照片後重試…');speak('手機儲存空間不足，系統正在壓縮照片後重新儲存。');const changed=await optimizeStoredPhotos();try{const ok=save();renderStorageStatus();if(changed){toast(`已壓縮 ${changed} 份照片並完成儲存`);window.shuangfaSpeak?window.shuangfaSpeak('照片壓縮完成，付款資料已儲存。','success'):speak('照片壓縮完成，付款資料已儲存。')}return ok}catch(err2){if(err2?.code!=='STORAGE_QUOTA'&&err2?.message!=='STORAGE_QUOTA')throw err2;alert('手機瀏覽器的儲存空間仍不足，因此這筆資料尚未存入。\n\n系統不會自動跳出備份檔，也不會刪除任何照片。請回到系統設定，按「立即完整備份」後再整理儲存空間。');speak('儲存空間仍然不足，請先到系統設定進行完整備份。');throw new Error('手機儲存空間不足；請先手動完整備份。')}}}
async function compressAllPhotosSafely(){if(!confirm('系統只會壓縮照片容量，不會刪除任何請款單、支票照片或簽名。確定開始？'))return;const before=dbSizeBytes();const changed=await optimizeStoredPhotos();try{save();const after=dbSizeBytes();renderStorageStatus();const saved=Math.max(0,before-after);toast(`已壓縮 ${changed} 份照片，沒有刪除資料`);window.shuangfaSpeak?window.shuangfaSpeak('照片壓縮完成，所有照片均已保留。','success'):speak('照片壓縮完成，所有照片均已保留。');alert(`壓縮完成。\n處理照片：${changed} 份\n節省空間：約 ${fmtSize(saved)}\n所有照片與簽名均完整保留。`)}catch(e){alert('壓縮後仍超過手機瀏覽器容量，但系統沒有刪除任何照片。請先匯出完整備份，再改用容量較大的裝置。')}}
...
function updateSignaturePaymentInfo(){
  ...
  const method=draft.method||'付款方式未設定';
...
let signatureReminderTimer=null,signatureReminderRepeat=null,signatureConfirmed=false;
function isSignaturePageActive(){return !!$('#signature')?.classList.contains('active')}
function stopSignatureReminder(){clearTimeout(signatureReminderTimer);clearInterval(signatureReminderRepeat);signatureReminderTimer=null;signatureReminderRepeat=null}
function stopCurrentSpeech(){try{if('speechSynthesis' in window)window.speechSynthesis.cancel()}catch{}}
...
$('#invoicePhoto').onchange=async e=>{const files=[...e.target.files];for(const f of files){const data=await fileData(f);const accepted=await confirmPhotoPreview(data);if(accepted)invoicePhotos.push(accepted)}e.target.value='';renderPhotos()};
$('#checkPhoto').onchange=async e=>{if(e.target.files[0]){const data=await fileData(e.target.files[0]);const accepted=await confirmPhotoPreview(data);if(accepted)checkPhoto=accepted}e.target.value='';renderPhotos()};
function renderPhotos(){$('#invoicePreview').innerHTML=invoicePhotos.map((x,i)=>`<div class="photo-thumb-wrap"><img src="${x}" data-preview-photo="${i}"><button type="button" class="photo-remove" data-remove-invoice="${i}">×</button></div>`).join('');$('#checkPreview').innerHTML=checkPhoto?`<div class="photo-thumb-wrap"><img src="${checkPhoto}"><button type="button" class="photo-remove" id="removeCheckPhoto">×</button></div>`:'';$$('[data-remove-invoice]').forEach(b=>b.onclick=()=>{invoicePhotos.splice(Number(b.dataset.removeInvoice),1);renderPhotos()});$('#removeCheckPhoto')?.addEventListener('click',()=>{checkPhoto='';renderPhotos()})}
$('#photosNext').onclick=()=>{updateSignaturePaymentInfo();signatureConfirmed=false;stopSignatureReminder();show('signature');setTimeout(()=>{if(isSignaturePageActive()&&!hasSignature)speak('請先確認收款金額無誤，再於下方簽名。')},350)};
...
function exportSignatureImage(){const out=document.createElement('canvas');out.width=c.width;out.height=c.height;const o=out.getContext('2d');o.fillStyle='#fff';o.fillRect(0,0,out.width,out.height);o.drawImage(c,0,0);return out.toDataURL('image/jpeg',.92)}
function sizeCanvas(){
